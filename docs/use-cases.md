...
- at the time of decisioning, he has one other payday loan, confirmed via KlearLink match data
...
- XYZ performs match data pull from KlearLink on 3rd payday loan application
...
- application match data is obtained via KlearLink
  - data returns two other active payday loans
  - Two other institutions linked in the last 60 days
- Quick Cash utilizes the data obtained from KlearLink to prompt users to link two other accounts.
...
- Speedy Dough Inc. chooses to obtain ongoing monitoring from KlearLink on new customers.
